...
Use the `/playwright-cli` skill (read and follow that skill) to navigate the live application and gather evidence. Do NOT use MCP playwright tools — always drive exploration through the skill.
...
- Always read and follow the `playwright-cli` skill for browser exploration — never use MCP playwright tools directly
